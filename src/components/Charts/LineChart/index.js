  ...
  useEffect(() => {
    ...
    switch (reportType) {
      case "all":
        ...
        break
      case "7":
        newDate = data.filter((value,index)=>index>data.length-8)
        break
      case "30":
        newDate = data.filter((value,index)=>index>data.length-31)
        break
      default:
        newDate = data
        break
    ...
    setOptions(generateOption(newDate))
  }, [data, reportType])
  ...
  return (
    <div>
      <ButtonGroup size='small' style={{ 'display': "flex", justifyContent: 'flex-end' }}>
        <Button color={reportType === 'all' ? 'secondary' : ''} onClick={() => setReportType('all')}>1 năm</Button>
